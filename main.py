...
def main():
    ...
    def _dock():
        ...
        if cur_pos_y + cur_block.start_pos.Y <= 0:
            ...
            # 计算消除
            remove_idxs = []
            # 每次消除只发生在方块停靠后，所以从当前方块着手去判断即可
            # 就像五子棋中判断胜负只在落子后，所以判断胜负只是从当前落子位置出发
            for _i in range(cur_block.start_pos.Y, cur_block.end_pos.Y + 1):
...
def _draw_info(screen, font, pos_x, font_height, score):
    print_text(screen, font, pos_x, 10, '得分: ')
    print_text(screen, font, pos_x, 10 + font_height + 6, '{}'.format(score))
    print_text(screen, font, pos_x, 20 + (font_height + 6) * 2, '速度: ')
    print_text(screen, font, pos_x, 20 + (font_height + 6) * 3, '{}'.format(score // 10000))
    print_text(screen, font, pos_x, 30 + (font_height + 6) * 4, '下一个: ')
